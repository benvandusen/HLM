...
Load packages (We can probably cut this list)
```{r message=FALSE, warning=FALSE}
library(tidyr)
library(lme4)
library(multcomp)
...
Graph of Model 3 predicted values with 95% confidence intervals
```{r}
ggplot(HLM3, aes(y=Q, fill=test )) +geom_bar(stat="identity", position = position_dodge(width=0.9)) + geom_errorbar(aes(ymax=UCL, ymin=LCL), position=position_dodge(0.9), width=0.5) + scale_fill_brewer(palette="Paired") +
...
ggplot(HLM3, aes(y=Q, x=group, fill=test )) +geom_bar(stat="identity", position = position_dodge(width=0.9)) + geom_errorbar(aes(ymax=UCL, ymin=LCL), position=position_dodge(0.9), width=0.5) + scale_fill_brewer(palette="Paired") +
ylab("Gain (% points)") + xlab("") +
  theme(legend.position = c(0.36,-0.15), legend.direction = "horizontal", legend.title = element_blank()) 
```
